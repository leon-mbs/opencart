...
* updatequantity()  Обновляет количество  товаров в магазине. Параметр data передает  обьект ключ-значение - артикул товара=>количество {"id01":2,"id02":67}
* updateprice() Обновляет цену товаров в магазине. Параметр data передает  обьект ключ-значение - артикул товара=>цена  {"id01":233.00,"id02":555.00}

* getproducts() Загружает с  магазина товары которых нет на  складе на случай если товары были  добавлены  сначала  в  магазин  
* getnames() Загружает (по  SKU) с  магазина наименования товаров для  обновления наименований  на складе    
* updatenames() Загружает (по  SKU) наименования товаров со  склада для  обновления в магазине. Передается в поле data как массив ключ значение - sku=>name      
