...
class ZStore extends \Opencart\System\Engine\Controller {
    ...
    public function getproducts() {
        ...
        if (!isset($this->session->data['api_id'])) {
            ...
            try{
                ...
                $sql="SELECT p.master_id,p.sku,p.price,p.image,pd.name, p.weight,p.weight_class_id, m.name as manufacturer,(select coalesce(max(category_id),0) from `" . DB_PREFIX . "product_to_category` pc where  pc.product_id = p.product_id    ) as  cat_id FROM `" . DB_PREFIX . "product` p  join  `" . DB_PREFIX . "product_description` pd on p.product_id=pd.product_id  left join  `" . DB_PREFIX . "manufacturer` m on p.manufacturer_id=m.manufacturer_id  WHERE p.status=1 and  pd.language_id={$language_id}  and p.product_id in(select product_id from " . DB_PREFIX . "product_to_store  where store_id={$store_id} ) ";
